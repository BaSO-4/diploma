...
\end{lstlisting}
kjer \lstinline{nil} prestavlja odsotnost vrednosti, torej prazen seznam. V prikazovanju primerov svoje implementacije bom za prazen seznam uporabljal konstruktor \lstinline{Empty}, 
ki ne sprejme nobenega argumenta. \lstinline{Element} pa predstavlja vrednost, ki je vsebovana v seznamu. Ta je seveda odvisna od tipa seznama. Če bi na primer hoteli seznam celih števil, 
...
se uporabijo rekurzivni tipi. V svoji implementaciji sem uporabil iso-recursive pristop, ker zahteva manj dela na prevajalniku, funkciji \lstinline{fold} in \lstinline{unfold} pa sta prepuščeni uporabniku.
...
Preverjevalnik tipov tukaj nima težke naloge. Če imamo vsoto tipov \(T_1 + T_2\) in želimo pokazati, da je \lstinline{inl $t_1$} tipa \lstinline{$T_1 + T_2$}, moramo le pokazati, da je 
\lstinline{$t_1$} tipa \lstinline{$T_1$}. Podobno velja za \lstinline{inr $t_2$}. Za \lstinline{case} moramo preveriti več stvari. Najprej, da je argument tipa \lstinline{$T_1$ + $T_2$}. Nato preverimo 
obe veji, kjer predpostavimo da sta arumenta \lstinline{x} in \lstinline{y} tipov \lstinline{$T_1$} in \lstinline{$T_2$}, v tem vrstnem redu. Preverjamo, da sta tipa \lstinline{$T_1$} in \lstinline{$T_2$} 
vrnjenih vrednosti enaka. Če je to res, označimo tip vrnjene vrednosti kot \lstinline{T}. Celoten izraz ima nato tip \lstinline{$T_1 + T_2$ $\rightarrow$ T}.

Ker v \lstinline{case} izrazu preverjamo tip izraza, ki je podan kot argument na način, da samo preverimo ali ustreza enemu od \emph{\(T_1\) in \(T_2\)} in se ne oziramo na drugi tip v vsoti, 
...
Tako se malenkost spremeni notacija. Namesto $T_1 + T_2$ pišemo $\langle l_1:T_1, ... , l_n:T_n\rangle$, kjer \(l_i\) stoji za i-to oznako, ali label. Prav tako namesto \lstinline{inl t as 
$T_1 + T_2$}, pišemo \lstinline{$\langle l_1$ = t$\rangle$ as $\langle l_1:T_1, ... , l_n:T_n\rangle$}. Tako naš primer z osebami postane:
\begin{lstlisting}
...
\end{lstlisting}
Elementi takega tipa bi nato bili oblike \lstinline{$\langle$deset=unit$\rangle$ as Ocena}. Tako lahko sestavimo tudi funkcije, ki obravnavajo take vrendnosti. Na primer;
\begin{lstlisting}
možno_zviševanje = λ.o:Ocena.
    ...
    | _         -> true;
\end{lstlisting}
je funkcija, ki sprejme eno izmed ocen in vrne odgovor na vprašanje, če jo je možno izboljšati. Tip te funkcije je \lstinline{Ocena $\rightarrow$ Bool}.
...
- : Stevilo = Naslednik (Naslednik (Naslednik (Nic)))
\end{lstlisting}
Definirali smo drevo \lstinline{lipa}, ki ima korensko vozlišče in dva lista. Funkcija \lstinline{st\_vozlisc} vrne število tri.
...
\end{lstlisting}
Definirajmo še izomorfizem \lstinline{fold}, ki nam bo omogočal, da bomo lahko slikali tipe \lstinline{Element $\rightarrow (Element \rightarrow Lacna)$} v \lstinline{Element $\rightarrow$ Lacna} 
